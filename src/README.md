...
| `EntidadBancaria` | 9080 | External bank confirmation agent |
| `Transportista` (×3) | 9071–9073 | External shippers with configurable negotiation behaviour |
| `EmpresaVendedora` (×4) | 9090–9093 | External sellers for delegated sales and product registration |
...
python EntidadBancaria.py --port 9080 --dir http://192.168.1.10:9000 --hostaddr 192.168.1.XX --open --config external_agents_config.json
python Transportista.py --port 9071 --dir http://192.168.1.10:9000 --hostaddr 192.168.1.XX --open --config external_agents_config.json --profile rapidship
python EmpresaVendedora.py --port 9090 --dir http://192.168.1.10:9000 --hostaddr 192.168.1.XX --open --config external_agents_config.json --profile homeplus
...
This starts all agents (DirectoryService, Logger, Client, Catalogador, Valorador, Tesorero, EntidadBancaria, 3 Transportista profiles, 4 EmpresaVendedora profiles, Ventas, and 4 CentroLogistico instances) as background processes. Press any key to stop them all cleanly.

The script auto-detects the Python interpreter in `env/`, `.venv/`, or falls back to the system `python`/`python3`.

External agents are configured in `external_agents_config.json`. Adjust that file to make transport companies more or less expensive, change their concession step, or add products/IBANs for external sellers.
...
```bash
cd src/
REGISTER_EXTERNAL_PRODUCTS=1 ./demo_tick.sh
```
...
curl http://127.0.0.1:9090/tick/nuevo-producto
```
...
├── EntidadBancaria.py    # External banking actor
├── Transportista.py      # External transport actor
├── EmpresaVendedora.py   # External seller actor
